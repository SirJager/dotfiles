...
# =================================================================================================================================
function _kdeCommands() {
  local file="$1"
 [[ -z "$file" ]] && file="$XDG_CONFIG_HOME/kdeconnect/4a86931ba7b04a50a8fca6477daa1371/kdeconnect_runcommand/config"
  grep '^commands=' "$file" \
    | sed -E 's/^commands="@ByteArray\(//' \
    | sed -E 's/\)"$//' \
    | sed 's/\\\\/\\/g; s/\\"/"/g' \
    | jq .
}
...
# =================================================================================================================================
function _tvsesh () {
  tmux new-window -n "Television:Sesh" "tv sesh"
}
# =================================================================================================================================
...
function lst() {
  ...
  # if .gitignore exists, convert it → glob
  if [[ -f .gitignore ]]; then
    local gi
    gi=$(grep -vE '^\s*#|^\s*$' .gitignore \
      | sed 's|/$||' \
      | tr '\n' '|')
    ignore="${ignore}|${gi}"
  fi
...
# =================================================================================================================================
function compress_protected() {
  local input="$1"
  local output="${2:-$(basename "$input").tar.zst.gpg}"
...
function shred_dir() {
  local dir
  dir="$(realpath "$1")" || return 1
  [[ "$dir" == "/" ]] && return 1
  [[ -d "$dir" ]] || return 1
  find "$dir" -xdev -type f -print0 |
    xargs -0 shred -u -n 7 -z
  find "$dir" -xdev -depth -type d -empty -delete
}
# =================================================================================================================================
